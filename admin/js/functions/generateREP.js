const bodyParmsTest = {
  DatosGenerales: {
    ...
    ReceptorEmailCCO: "",
    EmailMensaje:
      "Prueba de envío y generación de factura mediante rest api desde el servicio de timbrado de FacturoPorTi",
  },
  Encabezado: {
    Emisor: {
      RFC: "EKU9003173C9",
  ...
  Complemento: {
    ...
    PagosV20: {
      Pagos: [
        {
          FechaPago: "2019-05-21T01:03:05",
          FormaPago: "03",
          Moneda: "MXN",
          TipoCambio: 1,
          DocumentosRelacionados: [
            {
              IdDocumento: "599C5833-B9EB-4FDA-B677-F63EEE93D772",
              Serie: null,
              Folio: "3523",
              Moneda: "MXN",
              Equivalencia: 1,
              NumeroParcialidad: 1,
              ImporteSaldoAnterior: 116,
              ImportePagado: 50,
              ImporteSaldoInsoluto: 66,
              ObjetoDeImpuesto: "02",
              Impuestos: {
                Trasladados: [
                  {
      ...
      Totales: {
        TotalTrasladosBaseIVA16: 43.1,
        TotalTrasladosImpuestoIVA16: 6.9,
        MontoTotalPagos: 50,
      },
    },
  },
};
async function generateREP(id_pay) {
  console.log(id_pay);
  let dataPayment = await getPaymentDetails(id_pay);
  console.log(dataPayment.complementos);
  complementos = dataPayment.complementos;
  new_insoluto = dataPayment.new_insoluto;
  new_saldo_anterior = dataPayment.new_saldo_anterior;
  let data_receptor = await getReceptorData();
  let serie = await generateAlphaNumericSeries();
  let folio = await generateUniqueFolio();
  console.log(data_receptor);
  email_receptor = $("#email_receptor").val();
  //console.log(data);
  order_code = "AFAFAFA";
  id_order = "ADKD2313";
  var date = new Date(
...
async function getReceptorData() {
  ...
  const uso_cfdi = document.getElementById("select-uso-cfdi").value;
  const zipcode = document.getElementById("zipcode").value.trim();
  const reg_fiscal = document.getElementById("select-reg-fiscal").value;
  const street = document.getElementById("street").value.trim();
  const ext_num = document.getElementById("ext_num").value.trim();
  const int_num = document.getElementById("int_num").value.trim();
  ...
  // Construir el objeto data_receptor
  const data_receptor = {
    RFC: rfc,
...
async function getPaymentDetails(id_pay) {
  loading();
  return new Promise((resolve, reject) => {
    $.ajax({
      ...
      .done(function (data) {
        Swal.close();
        try {
          var parsedData = JSON.parse(data); // Parsear el JSON
          resolve(parsedData); // Resolviendo la promesa
        } catch (error) {
      ...
      .fail(function (message) {
        ...
        reject(message); // Rechazar la promesa en caso de error
      });
  });
}
// Función para generar una serie de 3 dígitos alfanuméricos
async function generateAlphaNumericSeries() {
  return new Promise((resolve) => {
    const characters = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
    let series = "";
    for (let i = 0; i < 3; i++) {
      series += characters.charAt(
        Math.floor(Math.random() * characters.length)
      );
    }
    resolve(series); // Retorna la serie como promesa resuelta
  });
}

// Función para generar un folio de 4 dígitos numéricos irrepetibles
async function generateUniqueFolio() {
  return new Promise((resolve) => {
    const timestamp = Date.now();
    const uniqueFolio = timestamp.toString().slice(-4); // Últimos 4 dígitos del timestamp
    resolve(uniqueFolio); // Retorna el folio como promesa resuelta
  });
}
function consumeAPI(
  bodyParms,
  ...
  const apiUrl = "https://testapi.facturoporti.com.mx/servicios/timbrar/json";
  ...
  const requestOptions = {
    ...
    body: JSON.stringify(bodyParmsTest), // Convertir los parámetros a formato JSON
  };
      ...
      $.ajax({
        ...
        .done(function (data) {
          ...
          if (data.response == true) {
            Swal.fire({
              title: "REP generado",
              text: "El complemento de pago ha sido generado correctamente",
              icon: "success",
              showCancelButton: false,
              confirmButtonColor: "#32a852",
              confirmButtonText: "Aceptar",
...
function downloadPDF(base64Data) {
  ...
  link.download = "REP.pdf"; // Nombre del archivo a descargar
  document.body.appendChild(link); // Agregar el enlace al DOM
  link.click(); // Hacer clic automáticamente en el enlace
...
function getTokenTest() {
  return "eyJhbGciOiJodHRwOi8vd3d3LnczLm9yZy8yMDAxLzA0L3htbGRzaWctbW9yZSNobWFjLXNoYTI1NiIsInR5cCI6IkpXVCJ9..fJ4fx_KHaUhqrtrm1wfXqSFT4ytasGF2hRubCUWhlZA";
}
$(document).ready(function () {
  // Inicialización de Select2
  function initializeSelect2(selector, parent, width = "resolve") {
    $(selector).select2({
  ...
  initializeSelect2("#selectState", "#modalReceptorData");
  initializeSelect2("#selectCity", "#modalReceptorData");
  initializeSelect2("#selectClient", "#modalReceptorData");

  // Función para manejar errores
  function handleError(message) {
    VanillaToasts.create({
      title: "Error",
      text: message || "Ocurrió un error, intentelo nuevamente",
      type: "error",
      timeout: 1200,
  ...
  // Función para rellenar opciones de un select
  ...
  // Manejar cambio en selectState
  $(document).on("change", "#selectState", async function () {
    ...
    if (id_estado) {
      ...
      try {
        const response = await $.ajax({
          ...
          method: "POST",
          data: { mod: "getMunicipios", id_estado },
        });
        const data = JSON.parse(response);
        $("#selectCity").prop("disabled", !data.response);
        if (data.response) {
          populateSelect(
            $("#selectCity"),
            data.data,
          ...
          Swal.fire({ icon: "error", title: "Verifique los datos ingresados" });
        }
      } catch (error) {
        handleError();
      } finally {
  ...
  // Manejar cambio en selectClient
  $(document).on("change", "#selectClient", async function () {
    const id_client = this.value;
    if (id_client) {
      loading();
      try {
        const response = await $.ajax({
          url: "php/controllers/colabs/colab_controller.php",
          method: "POST",
          data: { mod: "getClientBillingInfo", id_client },
        });
        const data = JSON.parse(response);
        if (data.response && data.data.length > 0) {
          const clientInfo = data.data[0];
          $("#razon_social").val(clientInfo.razon_social);
          $("#rfc").val(clientInfo.rfc);
          $("#email_receptor").val(clientInfo.email);
          $("#street").val(clientInfo.street);
          $("#ext_num").val(clientInfo.ext_number);
          $("#int_num").val(clientInfo.int_number);
          $("#colony").val(clientInfo.colony);
          $("#locality").val(clientInfo.locality);
          $("#zipcode").val(clientInfo.zip_code);
          $("#select-uso-cfdi").val(clientInfo.c_UsoCFDI);
          $("#select-reg-fiscal").val(clientInfo.c_RegimenFiscal);
          $("#selectState").val(clientInfo.state).trigger("change");

          // Simula un retraso para garantizar que el estado se cargue antes de configurar la ciudad
          await delay(800);
          $("#selectCity").val(clientInfo.city).trigger("change");
        } else {
          Swal.fire({ icon: "error", title: "Verifique los datos ingresados" });
        }
      } catch (error) {
  ...
  $(document).on("click", ".startGenREP", function () {
    const id_pay = $(this).attr("data-id-pay");
    $("#btnGenerarFactura").attr("data-id-pay", id_pay);
  });
  $(document).on("click", ".setPaymentMethodPay", function () {
    const id_pay = $(this).attr("data-id-pay");
    $("#updatePaymentPay").attr("data-id-pay", id_pay);
  });
  $(document).on("click", "#updatePaymentPay", function () {
    const id_pay = $(this).attr("data-id-pay");
    let id_payment_method = $("#slct-paymentMethodPay").val();
  ...
  function resetClientForm() {
    ...
    $(
      "#razon_social, #rfc, #email_receptor, #street, #ext_num, #int_num, #colony, #locality, #zipcode"
    ).val("");
