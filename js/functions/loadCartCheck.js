document.addEventListener("DOMContentLoaded", function () {
  ...
  const lblTotalCartShopMain = document.getElementById("lblTotalCartShopMain");
  ...
  function updateCart() {
    ...
    cart.forEach((item, index) => {
      ...
      row.innerHTML = `
        ...
        <td class="product-price align-middle">$${parseFloat(item.price).toFixed(2)}</td>
        ...
        <td class="product-total align-middle">$${(item.price * item.quantity).toFixed(2)}</td>
      ...
      sessionStorage.setItem("total_sale", total.toFixed(2));

    });

    // Actualizar total y contador de artículos
    cartItemCount.textContent = cart.length;
    cartTotal.textContent = `$${total.toFixed(2)}`;
    lblTotalCartShopMain.textContent = `$${total.toFixed(2)}`; // Actualiza lblTotalCartShopMain
  }
  ...
  document.addEventListener("click", function (event) {
    if (event.target.classList.contains("increase") || event.target.classList.contains("decrease")) {
      ...
      if (event.target.classList.contains("increase") && cart[index].quantity < stock) {
        cart[index].quantity++;
    ...
    if (event.target.classList.contains("remove-item")) {
      ...
      sessionStorage.setItem("cart_shop", JSON.stringify(cart));
      updateCart(); // Actualizar la tabla y el resumen
    }
  });
});
